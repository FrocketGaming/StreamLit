...
st.set_page_config(page_title="SQL Formatter", page_icon=":shark:")
...
selected = option_menu(
    menu_title=None,
    options=["SQL Formatter", "JSON Formatter", "XML Formatter", "CRON Placeholder"],
...
def sql_data_format(user_text, checked):
    if checked == True:
        updated_text = (
            "(" + ", ".join(repr(s) for s in user_text.replace(",", "").split()) + ")"
        )
        ...
        if "," in user_text or ", " in user_text:
            user_text.replace(",", " ")
            updated_text = ",".join(repr(s) for s in user_text.split(","))
            return textwrap.fill(updated_text, 65)
...
def column_formatter():
    ...
    with column[0]:
        ...
        selection = st.radio(
            "None",
            ("Format Data", "Format Query", "Remove Java"),
